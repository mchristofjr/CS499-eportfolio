...
class AnimalShelter:
    ...
    def read(self, query: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        """
        try:
            # If the caller passes None or a non-dict value, default to "match all documents"
    ...
    def update(self, query: Dict[str, Any], new_values: Dict[str, Any], many: bool = True) -> int:
        ...
        """
        # Defensive validation: require a non-empty filter and a non-empty update payload
        ...
        try:
            # many=True updates all matching documents; many=False updates only the first match
            if many:
